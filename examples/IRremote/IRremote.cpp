...
void IRsend::mark(int time) {
  ...
  uint32_t count = (uint32_t)time*(uint32_t)outputFreq/1000000UL;
  softwarePWMOut(count); // Enable PWM output
...
void IRsend::space(int time) {
  ...
  #if defined(__AVR__)
  time-=150; //The extra uint32 math seems to hurt on an 8 bit platform!
  #endif
  delayMicroseconds(time);
}
...
void IRsend::enableIROut(int khz) {
  ...
  digitalWrite(txPin, LOW); // When not sending PWM, we want it low
  
  outputFreq=khz*1000UL;
}
...
inline void tunedDelay(uint32_t delay) { 
  if ( delay == 0 )
  {
    return ;
  }
...
void IRsend::softwarePWMOut(uint32_t count){
  uint32_t pulsePeriod = (uint32_t)(VARIANT_MCK / outputFreq / 4UL); // 4=delay cycles
  volatile portType *reg = portOutputRegister(digitalPinToPort(txPin));
  portType reg_mask = digitalPinToBitMask(txPin);
  portType inv_mask = ~reg_mask;
  uint16_t delayTime = pulsePeriod/2UL - 4UL;
  while(count){
    *reg |= reg_mask;
    tunedDelay(delayTime);
    *reg &= inv_mask;
    tunedDelay(delayTime);
    count--;
  }
